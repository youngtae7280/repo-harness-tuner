...
def add_loop_arguments(command_parser: argparse.ArgumentParser) -> None:
    ...
    command_parser.add_argument("--domain", default="current repository")
    command_parser.add_argument("--module", action="append", help="Module human-involvement override, for example 'Ending taxonomy: 5'.")
    command_parser.add_argument("--human-involvement", type=int, choices=[1, 2, 3, 4, 5], help="User-facing intervention level: 1=minimal, 5=maximum.")
    command_parser.add_argument("--repo-type", default="unknown")
    command_parser.add_argument("--team-size", type=int, default=3)
    command_parser.add_argument("--skill-source", default="builtin,ecc", help="Comma-separated skill recommendation sources: builtin,ecc,all.")
    command_parser.add_argument("--skill-limit", type=int, default=3, help="Maximum skill recommendations to show, capped at 3.")
    command_parser.add_argument("--catalog-root", help="Optional local checkout for an external catalog such as ECC.")
    command_parser.add_argument("--write-plan", action="store_true", help="Write Docs/AI/harness-loop-plan.md.")
    command_parser.add_argument("--write-recommended", action="store_true", help="Apply the next recommended file-writing action.")
    command_parser.add_argument("--record-history", action="store_true", help="Append a run-loop snapshot to Docs/AI/harness-history.jsonl.")
    command_parser.add_argument("--note", default="")
    command_parser.add_argument("--force", action="store_true")
    command_parser.add_argument("--confirm-write", action="store_true", help="Confirm file writes when human involvement is 4 or 5.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_loop = sub.add_parser("run-loop", help="Run the full planning loop and choose the next safe action.")
    add_loop_arguments(run_loop)
    run_loop.set_defaults(func=cmd_run_loop)

    loop_alias = sub.add_parser("loop", help="Alias for run-loop.")
    add_loop_arguments(loop_alias)
    loop_alias.set_defaults(func=cmd_run_loop)

    next_alias = sub.add_parser("next", help="Friendly alias for run-loop; answers what Codex should do next.")
    add_loop_arguments(next_alias)
    next_alias.set_defaults(func=cmd_run_loop)
